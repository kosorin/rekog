...
namespace Rekog.Commands
{
    public class NgramCommand : ControllerCommand<NgramOptions, NgramController>
    {
        public NgramCommand() : base("ngram")
        {
            ...
            AddOption(new Option<string?>(new[] { "--pattern", "-p" }, () => "*", "Search string to match against the names of files in path (used only if input is directory); If starts with '/' then search in all subdirectories"));
            AddOption(new Option<FileInfo?>(new[] { "--output", "-o" }, "Output file path"));
            ...
            AddOption(new Option<FileInfo[]?>(new[] { "--alphabet", "-a" }) { Name = nameof(NgramOptions.Alphabets) });
